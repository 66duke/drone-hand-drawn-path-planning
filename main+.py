...
def visualize_final_result(polygon_points,offset_points, start_point, best_path, scan_width, save_path="drone_path_result.png"):
...
# ===================== 【新增】offset =====================
def offset_polygon(points_or_poly, offset):
    # 如果传入点数组，先转为闭合多边形
    if isinstance(points_or_poly, np.ndarray):
        poly = Polygon(points_or_poly)
        ...
        poly = points_or_poly
    # 向内偏移
    safe_polygon = poly.buffer(-offset, join_style=1)
    if safe_polygon.is_empty:
        raise ValueError("Offset too large，偏移量超过图形本身尺寸！")
    # 提取外轮廓坐标，转为numpy数组输出
    if safe_polygon.geom_type == 'MultiPolygon':
        safe_polygon = max(safe_polygon.geoms, key=lambda p: p.area)
    offset_coords = np.array(safe_polygon.exterior.coords)
...
if __name__ == '__main__':
    ...
    start_point = (900, 100)
    scan_width = 40
    offset_points = offset_polygon(original_points, scan_width*0.5)
    all_steps = drone_coverage_path(offset_points, start_point, scan_width)
    ...
    visualize_final_result(original_points,closed_sketch, start_point, best_path, scan_width)
