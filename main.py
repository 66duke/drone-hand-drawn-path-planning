...
def generate_parallel_scan_lines(polygon, theta, scan_width):
    ...
    # 2. 按固定间隔排布（修正：先判断，再append，保证所有线都在区域内）
    while True:
        # 当前线的右边界不能超过max_p
    ...
    if pos_queue:
        ...
        gap = max_p - (last_pos + H)  # 实际剩余空隙（最后一条线的右边界到区域右端）
        print(f"DEBUG: 最后一条线右边界: {last_pos + H:.1f}, 区域右端: {max_p:.1f}, 空隙: {gap:.1f}")

        # 空隙大于0，且空隙 > H，说明覆盖不到，需要补线
        if gap > 0 and gap > H:
            # 补线位置：last_pos + W/2（也就是last_pos + H，正好在空隙中间）
            add_pos = last_pos + H
    ...
    if final_group:
        ...
        right_space = max_p - (end_p + H)  # 用最后一条线的右边界来算
...
def generate_all_possible_paths(grouped_scan_lines):
    ...
    path3 = []
    reverse = True
    for group in reversed(grouped_scan_lines):
        if reverse:
            group = group[::-1]
            for line in group:
                path3.extend(line[::-1])
...
def drone_coverage_path(sketch_points, start_point, scan_width):
    ...
    for theta in candidate_thetas:
        ...
        if not grouped_lines:
            continue
        all_paths = generate_all_possible_paths(grouped_lines)
